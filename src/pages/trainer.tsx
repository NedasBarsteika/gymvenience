...
function TrainerPage() {
  ...
  const [trainerName, trainerSurname] = trainerSlug?.split("-") || [];
  const [trainer, setTrainer] = useState<{ name: string, surname: string } | null>(null);
  useEffect(() => {
    if (trainerName && trainerSurname) {
      axios.get(`https://localhost:7296/user/getTrainer?name=${trainerName}&surname=${trainerSurname}`)
        ...
        >
            <section className="py-6 bg-gray-100">
                ...
                <h1 className="text-4xl md:text-6xl font-bold">
                  {trainerName && trainerSurname
                    ? `${capitalize(trainerName)} ${capitalize(trainerSurname)} laikai`
                    : "Trenerio laikai"}
                </h1>
